...
export default async function AdminPaymentsPage(props: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  const resolvedParams = await (props.searchParams ?? Promise.resolve({} as { [key: string]: string | string[] | undefined }));
  const page = Number(resolvedParams?.page ?? "1");
  const { payments, total, page: currentPage, perPage } = await getAdminPayments({ page, perPage: 50, search: resolvedParams?.search as string | undefined });
    ...
    <div className="space-y-8">
      ...
      <Card>
        <form method="get" className="mb-4 flex gap-2">
          <input name="search" defaultValue={resolvedParams?.search ?? ""} placeholder="Search payments" className="w-full rounded-md border px-3 py-2" />
